...
var sass = function(config) {
  if (config.type === 'core') {
    return { test: /\.s[ac]ss$/, loaders: [ 'style-loader', 'css-loader', 'sass-loader' ] };
  } else {
    return {
      test: /\.s[ac]ss$/,
...
var image = function(config) {
  return {
    test: /\.(png|jpg|gif)$/,
    loader: 'file-loader?name=' + config.name + '/' + config.service + '/' + config.env +
...
var locale = function(config) {
  return {
    test: /locales\/.+\.json$/,
    loader: 'bundle-loader?name=' + config.name + '/' + config.service + '/' + config.env +
      '/locales/[name]',
    include: new RegExp('node_modules/' + config.name),
  };
};
